...
(defun available-defs-libraries ()
  (setf libs-defs nil)
  (verbose-command  (concat "ls " home-dir "definitions |  wc -l"))
  (let ((n nil)
	(i 0))
  (with-current-buffer "*Shell Command Output*"
    (beginning-of-buffer)
    (setq n (string-to-number (format "%s"  (read (current-buffer))))))
  (verbose-command  (concat "ls " home-dir "definitions"))
  (with-current-buffer "*Shell Command Output*"
...
(defun available-thm-libraries ()
  ...
  (verbose-command  (concat "ls " home-dir "theorems |  wc -l"))
  (let ((n nil)
	(i 0))
  (with-current-buffer "*Shell Command Output*"
    (beginning-of-buffer)
    (setq n (string-to-number (format "%s"  (read (current-buffer))))))
  (verbose-command  (concat "ls " home-dir "theorems"))
  (with-current-buffer "*Shell Command Output*"
    (progn (beginning-of-buffer)
	   (while (< i n)
	     (let ((r (format "%s" (read (current-buffer)))))
