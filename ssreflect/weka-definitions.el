...
(defun weka-defs ()
  (let ((alg (cond ((string= "k" algorithm)
    ...
    (shell-command (concat "tail -n +56 " (expand-file-name "out.arff")
                           " > "
        ...
        (if (or (not temp1)
          ...
          (progn
            ...
            (do ((temp2 temp1 (cdr temp2)))
                ((endp temp2))
              (if (not (string= (format "%s" (car (nth (- (car temp2)
                                                          1)
                                                       tables-definitions))
                                        )
                                (format "%s" name)))
...
(defun cluster-statements ()
  ...
  (weka-thms)
  (sleep-for 2)
  (print-clusters-weka-thms (cond  ((eq 2 granularity-level)
                                    (floor (length tables-thms)
                                           7))
                                   ((eq 3 granularity-level)
                                    (floor (length tables-thms)
