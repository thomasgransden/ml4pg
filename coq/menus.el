...
(defun available-libraries ()
  (verbose-command  (concat "ls " home-dir "libs/coq | grep .csv | wc -l"))
  (let ((n nil)
	(i 0))
  (with-current-buffer "*Shell Command Output*"
    (beginning-of-buffer)
    (setq n (string-to-number (format "%s"  (read (current-buffer))))))
  (verbose-command  (concat "ls " home-dir "libs/coq | grep .csv"))
  (with-current-buffer "*Shell Command Output*"
...
(defun available-dirs ()
  (verbose-command  (concat "ls -d " home-dir "libs/coq/*/ | wc -l"))
  (let ((n nil)
	(i 0))
  (with-current-buffer "*Shell Command Output*"
    (beginning-of-buffer)
    (setq n (string-to-number (format "%s"  (read (current-buffer))))))
  (verbose-command  (concat "ls -d " home-dir "libs/coq/*/"))
  (with-current-buffer "*Shell Command Output*"
    (progn (beginning-of-buffer)
	   (while (< i n)
	     (let ((r (format "%s" (read (current-buffer)))))
...
(defun libraries-dir (dir)
  (verbose-command  (concat "ls " home-dir "libs/coq/" dir "/ | grep _names | wc -l"))
  (let ((n nil)
	(i 0)
	(temp nil))
  (with-current-buffer "*Shell Command Output*"
    (beginning-of-buffer)
    (setq n (string-to-number (format "%s"  (read (current-buffer))))))
  (verbose-command  (concat "ls " home-dir "libs/coq/" dir "/ | grep _names"))
  (with-current-buffer "*Shell Command Output*"
    (progn (beginning-of-buffer)
	   (while (< i n)
