...
(defun get-numbers (cmd tactic ngs ts current-level bot)
  (cond ((search "- inv H" cmd)
         (list (cdr (assoc "inv" tactic_id))
               1 1 -1 ngs ngs)
         )
        ((or (string= cmd "2: eauto.")
             (string= cmd "3: eauto."))
         (if (assoc "eauto" tactic_id)
             (progn (setf goal-level-temp (cons  (list (cdr (assoc "eauto" tactic_id))
                                                       0 0 0 ts ngs)
                                                 goal-level-temp))
                    (list (cdr (assoc "eauto" tactic_id))
                          0 0 0 ts ngs))
           (progn
        ...
        ((string= tactic "intros")
         (list (cdr (assoc "intro" tactic_id))
               1 1 -1 ngs ngs)
         )
        ...
        ((or (string= (subseq cmd 0 (if (< 8 (length cmd))
                                        8 (length cmd)))
                      "intros [")
             (string= (subseq cmd 0 (if (< 7 (length cmd))
                                        7 (length cmd)))
                      "intros;")
             (string= (subseq cmd 0 (if (< 12 (length cmd))
                                        12 (length cmd)))
                      "intros until")
             (search ";intros" cmd))
         (list (cdr (assoc "intro" tactic_id))
        ...
        ((string= tactic "red.")
         (progn (setf goal-level-temp (cons  (list (cdr (assoc "red" tactic_id))
                                                   0 0 0 ts ngs)
                                             goal-level-temp))
                (list (cdr (assoc "red" tactic_id))
                      0 0 0 ts ngs))
         )
        (t (if (assoc tactic tactic_id)
               (progn (setf goal-level-temp (cons  (list (cdr (assoc tactic tactic_id))
                                                         0 0 0 ts ngs)
                                                   goal-level-temp))
                      (list (cdr (assoc tactic tactic_id))
                            0 0 0 ts ngs))
             (progn
