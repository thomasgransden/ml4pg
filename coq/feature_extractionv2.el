...
(defun get-numbers (cmd tactic ngs ts current-level bot)
  ...
  (cond ((search "- inv H" cmd)
        ...
        ((or (string= (subseq cmd 0 (max 8 (length cmd)))
                      "intros [")
             (string= (subseq cmd 0 (max 7 (length cmd)))
                      "intros;")
             (string= (subseq cmd 0 (max 12 (length cmd)))
                      "intros until")
             (search ";intros" cmd))
           (list (cdr (assoc "intro" tactic_id)) 1 1 -1 ngs ngs))
        ...
        ((string= tactic "rewrite")
           (add-info-to-tree (list 0 0 0 0 0 (extract-theorem-id cmd)
                                   0 1 0)
                             current-level)
        ...
        ((string= cmd "simpl; trivial.")
           (add-info-to-tree (list 0 0 ts 0 0 0 0 1 1)
                             current-level)
           (add-info-to-tactic (list 0 0 ts 1)
                               "simpltrivial")
