...
;; signal is used to indicate the function which has called to matlab and to process the result
    ...
    (with-current-buffer "*display*"
      (progn (erase-buffer)
             (cond ((equal signal 0)
                   ...
                   ((equal signal 3)
                    (compute-clusters-and-values (split-clusters-aux (remove-jumps (subseq my-buffer (search "load" my-buffer :from-end t)))
                                                                     nil)

                                                 (split-frequencies (remove-jumps  (subseq my-buffer (search "load" my-buffer :from-end t)))
                                                                    nil)))
                   (t nil)))))
  output)
        ...
        (let ((end (search "]" str :start2 (1+ init))))
          (if (not (search "char" (subseq str init end)))
              (split-frequencies (subseq str (1+ end))
                                 (cons (string-to-number (remove-jumps (subseq str (1+ init)
                                                                               end)))
                                       res))
            (split-frequencies (subseq str (1+ (search "[" str :start2 (1+ end))))
                               res)
            ))
      res)))
    ...
    (let* ((clu-freq (extract_clusters_freq (read-lines (expand-file-name "matlab_res.txt"))))
      ...
      (do ((temp res1 (cdr temp))
        ...
        (progn (insert (format "Cluster %s with frequency %s%%\n" i (car temp-freq)))
               ...
               (do ((temp2 (cluster-string-to-list (car temp))
                           (cdr temp2)))
                   ((endp temp2)
                    (insert (format "\n")))
                 (if (< (string-to-number (car temp2))
    ...
    (with-current-buffer "*display*"
      ...
      (do ((temp res1 (cdr temp))
        ...
        (progn (insert (format "Cluster %s: (" i ))
               (ignore-errors (insert-button-automaton2 (which-lemmas-in-cluster (car temp))
                                                        (car temp)))
               ...
               (do ((temp2 (car temp)
                 ...
                 (if (<= (car temp2)
                     ...
                     (ignore-errors (progn (insert (format "Lemma "))
                                           (insert-button-lemma (remove_last_colon
                                                                 (remove-jumps (car (nth (1- (car temp2))
                                                                                         saved-theorems)))))
                                           (insert (format " (%s)\n" (which-patch (1- (car temp2))
                                                                                  1)))))
                   (ignore-errors (progn (shell-command (concat "cat "(expand-file-name "names_temp.txt")
                                                                " | sed -n '"
                                                                (format "%s" (- (car temp2)
                                                                                (length saved-theorems)))
                                                                "p'"))
...
(defun show-clusters-of-theorem-iterative ()
  ...
  (let* ((alg (cond ((string= "g" algorithm)
                     "find_cluster_with_gaussian")
                    (t "find_cluster_with_kmeans")))
         (gra (if (not iterative)
                  (cond  ((eq 2 granularity-level)
                          5)
                         ...
                         ((eq 5 granularity-level)
                          20)
                         (t 3))
                (cond  ((eq 2 granularity-level-temp)
                        5)
                       ((eq 3 granularity-level-temp)
                        10)
                       ((eq 4 granularity-level-temp)
                        15)
                       ((eq 5 granularity-level-temp)
                        20)
                       (t 3)))))
    (progn (setf signal 1)
             ...
             (progn (setf signal 5)
                    ...
                    (weka (cond  ((eq 2 granularity-level)
                                 ...
                                 (t (floor (size-temp)
                                           8))))
                    (sleep-for 1)
                    (print-similarities-weka (cond  ((eq 2 granularity-level)
                                                     (floor (size-temp)
                                                            7))
                                                    ((eq 3 granularity-level)
                                                     (floor (size-temp)
