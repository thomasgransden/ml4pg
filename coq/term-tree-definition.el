...
(defun variables-fun (term)
  (do ((posop (search "(" term))
      ...
      ((not posop)
       (if (search ":" temp0)
           (subseq temp0 0 (search ":" temp0))
         temp2))
    (progn (setf temp2 (concatenate 'string temp2 (subseq temp0 (1+ posop)
                                                          (search ":" temp0))))
           (setf temp0 (subseq temp0 (1+ (search ":" temp0))
                               ))
...
(defun variables-fun-fix (term)
  (concatenate 'string (variables-fun (subseq term (+ 3 (search "fun" term))
                                              (search "=>" term)))
               " "
...
(defun definition-to-list (term)
  (cond ((search "fix " term)
         (list (definition-to-list-fix term)
               (definition-variables term)))
        ((search "let " term)
         (list (definition-to-list-let term)
               nil))
        ((search "fun " term)
         (list (definition-to-list-fun term)
               (definition-variables term)))
        (t (list (definition-to-list-aux term)
                 nil))))
...
(defun adddefinition (name)
  ...
  (let ((iftable (send-coq-cmd (format "Print Table Printing If.")))
    ...
    (setf listofdefinitions (append listofdefinitions
                                    (list (list 'definition (make-symbol name)
...
(defun transform-definitions ()
  ...
  (do ((temp definitions-libraries (cdr temp))
    ...
    (setf tables-definitions (append tables-definitions
                                     (list (build-table (extract-info (car temp)
                                                                      (car temp2))))))))
