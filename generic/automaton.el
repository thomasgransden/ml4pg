...
(defun extract-theorem-proof2-aux (type file name)
  (with-temp-buffer
    (insert-file-contents (concat home-dir "exported-libs/" type "/" file ".v"))
    (let* ((b (beginning-of-buffer))
           (l (search-forward name))
           (beg (point))
           (e (search-forward "Qed."))
...
(defun generate-automaton (l)
  (let ((d (concat home-dir "automaton/")))
    (with-current-buffer (buffer-name buf)
      (save-trace l))
    (with-temp-file
        (concat d "obtaintrace")
      (insert (format
               "#!/bin/bash
